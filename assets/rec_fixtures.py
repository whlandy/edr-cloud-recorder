"""回放用的 pytest fixture（可被多个 conftest 共享导入）。

本文件不叫 conftest.py，是为了能被 import —— pytest 对 conftest.py 有特殊处理，
非 rootdir 的 conftest 里也不允许再用 pytest_plugins（pytest 7+）。
真正的 conftest.py 只做一件事：from rec_fixtures import *。
...
def _auth_dir() -> Path:
    ...
    原来写死成「本文件同目录 / .auth」。在用户工程里那是对的（conftest 就在根目录），
    但技能仓库里 assets/ 只是个子目录，于是 recordings/ 下的草稿去 assets/.auth
    找登录态 —— 那里什么都没有，报错却是「没有可用的登录态」，指不到真正的原因。

    顺序：REC_STATE_DIR（录制器也认它）→ 当前工作目录下的 .auth → 本文件同目录。
    """
    env = os.environ.get("REC_STATE_DIR")
    if env:
        return Path(env).resolve()
    cwd = Path(".auth").resolve()
    if cwd.exists():
        return cwd
    return HERE / ".auth"
...
TEST_RESULTS = Path("test-results").resolve()
...
# conftest 之间共享用。`from rec_fixtures import *` 不会带上下划线开头的名字，
# 而 _auth_artifact_guard 是 autouse fixture —— 不显式列出来就会被悄悄丢掉。
__all__ = [
